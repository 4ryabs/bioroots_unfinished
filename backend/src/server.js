  ...
  io.on('connection', (socket) => {
    ...
    socket.on('pesanan_diklaim', () => {
      socket.broadcast.emit('payment_completed');
    });

    socket.on('gudang_diklaim', () => {
      socket.broadcast.emit('packing_completed');
    });

    socket.on('pesanan_baru_dibuat', () => {
      socket.broadcast.emit('system_alert', { type: 'NEW_DRAFT' });
    });
